...
class Book(Base):
    ...
    Note
    ----
    Uniqueness for ``user_book_id`` is enforced. Otherwise data is assumed to have
    passed through the Pydantic validation layer. Column level validation
    (e.g. ``nullable``) is applied for additional security and performance.

    There is some validation that may seem to occur, but doesn't:

    - Type hints e.g. ``Mapped[int]`` and character limits e.g. ``String(511)`` as are
      not enforced at runtime by SQLAlchemy. The underlying database dialect may - or
      may not - enforce them. SQLite is particularly permissive and enforces neither
      datatype nor character limits.

    - Missing fields are accepted and default to None. This only results in an error
      when committing to the database and only if the field is ``nullable=False``
    ...
    user_book_id: Mapped[int] = mapped_column(unique=True, nullable=False)
    ...
    book_tags: Mapped[Optional[List[str]]] = mapped_column(String, nullable=True)

    # Not required. Enables for ORM level navigation e.g. ``Book.highlights``.
...
class Highlight(Base):
    ...
    # Not required. Enables for ORM level navigation e.g. ``Highlight.book``.
